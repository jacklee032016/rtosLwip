...
static void tcpip_init_done(void *arg)
{
	...
	IP4_ADDR(&gw, 192,168,166,1);
	IP4_ADDR(&ipaddr, 192,168,166, 3);
	IP4_ADDR(&netmask, 255,255,255,0);

	netif_set_default(netif_add(&netif_tap, &ipaddr, &netmask, &gw, NULL, tapif_init,  tcpip_input));
	...
	IP4_ADDR(&netmask, 255,255,255,0);
	netif_set_default(netif_add(&netif_unix, &ipaddr, &netmask, &gw, NULL, unixif_init_server, tcpip_input));
	netif_set_up(&netif_unix); 
#if LWIP_IPV6
	netif_create_ip6_linklocal_address(&netif_unix, 1);
#endif

//	if(system("route add 192.168.167.1 192.168.166.3") != 0)
	if(system("route add 192.168.167.1 tap0") != 0)
	{
		return;
	}

//	if(system("route add 192.168.167.2 192.168.166.3") != 0)
	if(system("route add 192.168.167.2 tap0") != 0)
	{
		return;
	}

...
char cmnCmdLwipIgmp(const struct _EXT_CLI_CMD *cmd,  char *outBuffer, size_t bufferLen )
{
	...
	else if(strcasecmp(argv[1], "leave") == 0 )
	{
		ret = 2;
	}
	else //if( ret == 0 )
...
char	cmnCmdHttpClient(const struct _EXT_CLI_CMD *cmd,  char *outBuffer, size_t bufferLen)
{
	...
	if(argc < 4)
	{
		index += snprintf(outBuffer+index, bufferLen-index, EXT_NEW_LINE " only %d arguments :", argc);
		index += snprintf(outBuffer+index, bufferLen-index, EXT_NEW_LINE " IP Port URI :"EXT_NEW_LINE" Request SDP or JSON data"EXT_NEW_LINE);
		return EXT_FALSE;
	}
...
char cmdCmdDebuggable(const struct _EXT_CLI_CMD *cmd,  char *outBuffer, size_t bufferLen )
{
	...
	if(argc < 2)
	{
		index += snprintf( outBuffer+index, (bufferLen-index), "'debug' configuration is '%08X'"EXT_NEW_LINE, extRun.debugOptions);
...
char	cmnCmdLocalInfo(const struct _EXT_CLI_CMD *cmd,  char *outBuffer, size_t bufferLen)
{
	...
	{
		index += snprintf(outBuffer+index, bufferLen-index, "'info' : multicast address can be used as local address for TX!"EXT_NEW_LINE);
		return EXT_FALSE;
	}

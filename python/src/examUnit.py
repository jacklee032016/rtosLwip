...
# begin with test_, and end with ".py"
# testfile = re.compile("^test_.*\.py$", re.IGNORECASE)

# "\." : escape '.'; "py$": end with py; ".*": any mumber of any characters. So match extension
fileExtension = re.compile(".*\.py$", re.IGNORECASE)

#
testfile = re.compile("^unit_.*\.py$")
imported = {}
...
def get_tests(directory=None):
    ...
    for name in sorted(os.listdir(directory)):
        ...
        if os.path.isdir(fullname):
            #print("enter subdirectory: "+name)
            tests.extend(get_tests(fullname))
        #elif fileExtension.match(name):
        #    print("ext match: " + name)
        elif testfile.match(name):
            modname = os.path.splitext(name)[0]
            print("mode match:\t" + modname)

...
//	unsigned char *d = data;
	...
	for(i=0; i< size; i++)
	{
		...
		{
			MUX_ERRORF(("Write RS232 failed on #%u", i));
			return EXIT_FAILURE;
		}
#if MUX_RS232_DEBUG
		printf("%c ", *(data+i));
...
void muxHwRs232Config(MUX_RUNTIME_CFG *runCfg)
{
	...
	printf("RS232: Parity:");
#endif
	if(runCfg->rs232Cfg.parityType == MUX_RS232_PARITY_ODD )
	{
	...
	else if(runCfg->rs232Cfg.parityType == MUX_RS232_PARITY_EVEN )
	{
...
#if MUX_RS232_DEBUG
	printf(MUX_NEW_LINE": CharBits:");
#endif
	if(runCfg->rs232Cfg.charLength == MUX_RS232_CHAR_LENGTH_5 )
	{
	...
	else if(runCfg->rs232Cfg.charLength == MUX_RS232_CHAR_LENGTH_6 )
	{
	...
	else if(runCfg->rs232Cfg.charLength == MUX_RS232_CHAR_LENGTH_7 )
	{
#if MUX_RS232_DEBUG
		printf("7");
#endif
...
#if MUX_RS232_DEBUG
	printf(MUX_NEW_LINE": StopBits:");
#endif
	if(runCfg->rs232Cfg.stopbits == MUX_RS232_STOP_BITS_2 )
	{
#if MUX_RS232_DEBUG
		printf("2");
#endif
	...
	else if(runCfg->rs232Cfg.stopbits == MUX_RS232_STOP_BITS_15 )
	{
#if MUX_RS232_DEBUG
		printf("1.5");
#endif
...
#if MUX_RS232_DEBUG
	printf(MUX_NEW_LINE"baudrate:%ld"MUX_NEW_LINE, _settings.baudrate );
#endif	
...
void muxHwRs232Init(MUX_RUNTIME_CFG *runCfg)
{
	...
	/* Configure USART */
	muxHwRs232Config(runCfg);

	muxRs232Write((unsigned char *)"RS232 OK", 8);
}

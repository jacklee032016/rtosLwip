/*
 *      xyzModem.c
 * stream handler for xyzModem protocol
 *
 */
#include <compact.h>
#include <xyModem.h>
...
#define NAK 0x15
#define CAN 0x18
#define XMODEM_EOF 0x1A		/* ^Z for DOS officionados */
...
/* Parse (scan) a number */
static bool parse_num (char *s, unsigned long *val, char **es, const char *delim)
{
	bool first = true;
	int radix = 10;
	char c;
				...
				if (stat == 0)
				{
					if (xyz.blk == xyz.next_blk)
					{
						...
						if (xyz.mode == xyzModem_xmodem || xyz.file_length == 0)
						{
							...
							if ((xyz.bufp[xyz.len - 1] == XMODEM_EOF) && (xyz.bufp[xyz.len - 2] == XMODEM_EOF) &&
								(xyz.bufp[xyz.len - 3] == XMODEM_EOF))
							{
								while (xyz.len && (xyz.bufp[xyz.len - 1] == XMODEM_EOF))
								{
									xyz.len--;
								}
							}
...
{
	diag_printf("xyzModem - %s mode, %d(SOH)/%d(STX)/%d(CAN) packets, %d retries\n\r", 	xyz.crc_mode ? "CRC" : "Cksum", xyz.total_SOH, xyz.total_STX, xyz.total_CAN, xyz.total_retries);
	ZM_DEBUG (zm_flush ());
}

...
static int unix_socket_client(const char *name)
{
	int fd;
...
static int unix_socket_server(const char *name)
{
	int fd;
#if !defined(LWIP_UNIX_LINUX) && !defined(LWIP_UNIX_CYGWIN) && !defined(__CYGWIN__)
	int len;
...
static void unixif_input_handler(void *data)
{
	struct netif *netif;
...
static err_t unixif_output(struct netif *netif, struct pbuf *p, const ip4_addr_t *ipaddr)
{
	struct unixif *unixif;
	...
	if (list_elems(unixif->q) == 0)
	{
		...
		sys_timeout((double)p->tot_len * 8000.0 / UNIXIF_BPS, unixif_output_timeout, netif);
...
static void unixif_output_timeout(void *arg)
{
	...
	if (list_elems(unixif->q) > 0)
	{
		sys_timeout(((struct unixif_buf *)list_first(unixif->q))->tot_len * 8000.0 / UNIXIF_BPS, unixif_output_timeout, netif);
	}
}

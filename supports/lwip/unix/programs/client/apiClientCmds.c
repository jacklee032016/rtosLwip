...
//static int apiClientUnicastSetParams(API_CLIENT *apiClient, char *_ipaddress, EXT_VIDEO_CONFIG * vCfg)
int apiClientSetupSys(struct API_CLIENT_CMD_HANDLER *handle, API_CLIENT *apiClient)
{
//	unsigned int ipAddress;
	EXT_VIDEO_CONFIG * vCfg = &apiClient->params->vCfg;
	char *data;
	...
	if(vCfg->ip != IPADDR_NONE)
	{
		...
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_IP"\":\"%s\",", inet_ntoa((*(struct in_addr *)&(vCfg->ip)) ));
	}
	
	if(apiClient->params->isDhcp != 0)
	{
		printf("DHCP: '%d'\n", apiClient->params->isDhcp-1);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_DHCP"\":%d,", apiClient->params->isDhcp-1 );
	}

	if(apiClient->params->isDipSwitch != 0)
	{
		printf("DIP Switch: '%d'\n", apiClient->params->isDipSwitch -1);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_IS_DIP"\":%d,", apiClient->params->isDipSwitch-1 );
	}
	
	if(strlen(apiClient->params->name))
	{
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_C_NAME"\":\"%s\"", apiClient->params->name);
	}
...
int apiClientSetupRs232(struct API_CLIENT_CMD_HANDLER *handle, API_CLIENT *apiClient)
{
	...
	if(apiClient->params->rs232bps != 0 )
	{
		...
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_RS_BAUDRATE"\":%d,", apiClient->params->rs232bps);
	}
	
	if(apiClient->params->rs232data != 0)
	{
		printf("Data: '%d'\n", apiClient->params->rs232data );
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_RS_DATABITS"\":%d,", apiClient->params->rs232data );
	}

	if(apiClient->params->rs232stop != 0)
	{
		printf("Stop: '%d'\n", apiClient->params->rs232data );
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_RS_STOPBITS"\":%d,", apiClient->params->rs232stop);
	}
	
	if(strlen(apiClient->params->rs232Parity))
	{
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_RS_PARITY"\":\"%s\"", apiClient->params->rs232Parity);
	}
...
int apiClientSetupProtocol(struct API_CLIENT_CMD_HANDLER *handle, API_CLIENT *apiClient)
{
	...
	if(apiClient->params->isMcast != 0 )
	{
		...
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_IS_MCAST"\":%d,", apiClient->params->isMcast -1 );
	}
	
	if(apiClient->params->vCfg.ip !=  IPADDR_NONE)
	{
		printf("MCAST IP: '%s'\n", inet_ntoa((*(struct in_addr *)&(apiClient->params->vCfg.ip))) );
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_MCAST_IP"\":\"%s\",", inet_ntoa((*(struct in_addr *)&(apiClient->params->vCfg.ip)))  );
	}

	if(apiClient->params->vCfg.vport != 0)
	{
		printf("V.port: '%d'\n", apiClient->params->vCfg.vport);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_PORT"\":%d,", apiClient->params->vCfg.vport);
	}

	if(apiClient->params->vCfg.aport != 0)
	{
		printf("A.port: '%d'\n", apiClient->params->vCfg.aport);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_AUDIO_PORT"\":%d,", apiClient->params->vCfg.aport);
	}

	if(apiClient->params->vCfg.dport != 0)
	{
		printf("D.port: '%d'\n", apiClient->params->vCfg.dport);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_AD_PORT"\":%d,", apiClient->params->vCfg.dport);
	}

	if(apiClient->params->vCfg.sport != 0)
	{
		printf("S.port: '%d'\n", apiClient->params->vCfg.sport);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_ST_PORT"\":%d", apiClient->params->vCfg.sport);
	}
...
int apiClientSetupMedia(struct API_CLIENT_CMD_HANDLER *handle, API_CLIENT *apiClient)
{
	...
	if(mediaParam->vWidth!= 0 && mediaParam->vHeight!= 0)
	{
		...
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_WIDTH"\":%d,", mediaParam->vWidth);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_HEIGHT"\":%d,", mediaParam->vHeight);
	}

	if(mediaParam->vFrameRate!= 0 )
	{
		printf("vFps: '%d'\n", mediaParam->vFrameRate );
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_FRAMERATE"\":%d,", mediaParam->vFrameRate);
	}
	
	if(strlen(apiClient->params->rs232Parity))
	{
		printf("vColorSpace: '%s'\n", apiClient->params->rs232Parity );
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_COLORSPACE"\":\"%s\",", apiClient->params->rs232Parity);
	}

	if(mediaParam->vDepth != 0 )
	{
		printf("vDepth: '%d'\n", mediaParam->vDepth);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_DEPTH"\":%d,", mediaParam->vDepth);
	}
	if(mediaParam->vIsInterlaced!= 0)
	{
		printf("vInterlaced: '%d'\n", mediaParam->vIsInterlaced-1);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_INTERLACED"\":%d,", mediaParam->vIsInterlaced-1 );
	}
	if(mediaParam->vIsSegmented != 0)
	{
		printf("vSegment: '%d'\n", mediaParam->vIsSegmented-1);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_VIDEO_SEGMENTED"\":%d,", mediaParam->vIsSegmented-1 );
	}

	if(mediaParam->aSampleRate != 0 )
	{
		printf("aFre: '%d'\n", mediaParam->aSampleRate );
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_AUDIO_SAMPE_RATE"\":%d,", mediaParam->aSampleRate);
	}
	if(mediaParam->aChannels!= 0 )
	{
		printf("aChannel: '%d'\n", mediaParam->aChannels);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_AUDIO_CHANNELS"\":%d,", mediaParam->aChannels);
	}

	if(mediaParam->aDepth != 0 )
	{
		printf("aDepth: '%d'\n", mediaParam->aDepth);
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_AUDIO_DEPTH"\":%d", mediaParam->aDepth);
	}
...
TRACE();
	...
	index += snprintf(data+index, size-index, "\""EXT_IPCMD_DATA_IS_CONNECT"\":\"%d\"", 1);
...
//	unsigned int ipAddress;
	...
	index += snprintf(data+index, size-index, "\""EXT_IPCMD_RS232_DATA_HEX"\":\"%02X%02X%02X%02X%02X%02X\",", 
	...
	index += snprintf(data+index, size-index, "\""EXT_IPCMD_RS232_FEEDBACK"\":%d,", (apiClient->params->isFeed==0)?0:1);
	index += snprintf(data+index, size-index, "\""EXT_IPCMD_RS232_WAIT_TIME"\":%d", apiClient->params->waitMs );
...
//	unsigned int ipAddress;
	...
	{
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_SC_SET_KEY"\":\"%02X%02X%02X%02X%02X%02X%02X%02X\"", 
			apiClient->params->hexData.address[0], apiClient->params->hexData.address[1], apiClient->params->hexData.address[2],
	...
	{
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_SC_GET_ID"\":\"\"" );
	}
	else if(apiClient->params->isGetStatus)
	{
		index += snprintf(data+index, size-index, "\""EXT_IPCMD_SC_GET_STATUS"\":\"\"" );
	}
	else 
	{

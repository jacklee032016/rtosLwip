...
#define NETIF_TO_HOST(netif)		(struct mdns_host*)(netif_get_client_data(netif, mdns_netif_client_id))	/* save struct mdns_host */
...
static u16_t mdns_readname_loop(struct pbuf *p, u16_t offset, struct mdns_domain *domain, unsigned depth)
{
	...
	do
	{
		...
		if((c & 0xc0) == 0xc0)
		{
			...
			LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: compressed label at offset:%d"LWIP_NEW_LINE, offset-1));
			if (offset >= p->tot_len)
			{
...
//	CONSOLE_DEBUG_MEM(domain->name, (uint32_t)domain->length, 0, "MDNS Domain Name");
	while (*src)
	{
		u8_t label_len = *src;
		src++;
...
static err_t mdns_read_rr_info(struct mdns_packet *pkt, struct mdns_rr_info *info)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: offset %d is qType: %d"LWIP_NEW_LINE, pkt->parse_offset-2, info->type));
...
err_t mdns_read_answer(struct mdns_packet *pkt, struct mdns_answer *answer)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: ANswer offset :%d"LWIP_NEW_LINE,pkt->parse_offset ));
...
static err_t mdns_add_a_answer(struct mdns_outpacket *reply, u16_t cache_flush, struct netif *netif)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with A record"LWIP_NEW_LINE));
...
static err_t mdns_add_hostv4_ptr_answer(struct mdns_outpacket *reply, u16_t cache_flush, struct netif *netif)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with v4 PTR record"LWIP_NEW_LINE));
...
static err_t mdns_add_aaaa_answer(struct mdns_outpacket *reply, u16_t cache_flush, struct netif *netif, int addrindex)
{
	struct mdns_domain host;
	mdns_build_host_domain(&host, NETIF_TO_HOST(netif));
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with AAAA record"LWIP_NEW_LINE));
...
static err_t mdns_add_hostv6_ptr_answer(struct mdns_outpacket *reply, u16_t cache_flush, struct netif *netif, int addrindex)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with v6 PTR record"LWIP_NEW_LINE));
	return mdns_add_answer(reply, &revhost, DNS_RRTYPE_PTR, DNS_RRCLASS_IN, cache_flush, (NETIF_TO_HOST(netif))->dns_ttl, NULL, 0, &host);
}
#endif
...
static err_t mdns_add_servicetype_ptr_answer(struct mdns_outpacket *reply, struct mdns_service *service)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with service type PTR record"LWIP_NEW_LINE));
...
static err_t mdns_add_servicename_ptr_answer(struct mdns_outpacket *reply, struct mdns_service *service)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with service name PTR record"LWIP_NEW_LINE));
...
static err_t mdns_add_srv_answer(struct mdns_outpacket *reply, u16_t cache_flush, struct mdns_host *mdns, struct mdns_service *service)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with SRV record"LWIP_NEW_LINE));
	return mdns_add_answer(reply, &service_instance, DNS_RRTYPE_SRV, DNS_RRCLASS_IN, cache_flush, service->dns_ttl, (const u8_t *) &srvdata, sizeof(srvdata), &srvhost);
}
...
static err_t mdns_add_txt_answer(struct mdns_outpacket *reply, u16_t cache_flush, struct mdns_service *service)
{
	...
	LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Responding with TXT record"LWIP_NEW_LINE));
	return mdns_add_answer(reply, &service_instance, DNS_RRTYPE_TXT, DNS_RRCLASS_IN, cache_flush, service->dns_ttl, (u8_t *) &service->txtdata.name, service->txtdata.length, NULL);
}
...
static void mdns_send_outpacket(struct mdns_outpacket *outpkt)
{
  ...
  if (outpkt->host_replies & REPLY_HOST_PTR_V6) {
	...
	if (outpkt->pbuf)
	{
		...
		/* Send created packet */
		LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Sending packet, len=%d, unicast=%d"LWIP_NEW_LINE, outpkt->write_offset, outpkt->unicast_reply));
...
static void mdns_handle_question(struct mdns_packet *pkt)
{
	...
	while (pkt->questions_left)
	{
		...
		if (res != ERR_OK)
		{
			LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Failed to parse question, skipping query packet"LWIP_NEW_LINE));
			return;
		}
...
#endif
		LWIP_DEBUGF(MDNS_DEBUG, (" type %d class %d"LWIP_NEW_LINE, q.info.type, q.info.klass));
	...
	while (pkt->answers_left)
	{
		...
		if (res != ERR_OK)
		{
			LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Failed to parse answer, skipping query packet"LWIP_NEW_LINE));
			goto cleanup;
		}
...
#endif
		LWIP_DEBUGF(MDNS_DEBUG, (" type %d class %d"LWIP_NEW_LINE, ans.info.type, ans.info.klass));
			...
			*/
			if (ans.info.type == DNS_RRTYPE_PTR)
			{
				...
				if (len != MDNS_READNAME_ERROR && res == ERR_OK && mdns_domain_eq(&known_ans, &my_ans))
				{
					...
					{
						LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: v4 PTR"LWIP_NEW_LINE));
					...
					{
						LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: v6 PTR"LWIP_NEW_LINE));
			...
			{
#if LWIP_IPV4
				if (ans.rd_length == sizeof(ip4_addr_t) && pbuf_memcmp(pkt->pbuf, ans.rd_offset, netif_ip4_addr(pkt->netif), ans.rd_length) == 0)
				{
					LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: A"LWIP_NEW_LINE));
			...
			{
#if LWIP_IPV6
			if (ans.rd_length == sizeof(ip6_addr_t) && /* TODO this clears all AAAA responses if first addr is set as known */
				pbuf_memcmp(pkt->pbuf, ans.rd_offset, netif_ip6_addr(pkt->netif, 0), ans.rd_length) == 0)
			{
				LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: AAAA"LWIP_NEW_LINE));
				...
				*/
				if (ans.info.type == DNS_RRTYPE_PTR)
				{
					...
					if (len != MDNS_READNAME_ERROR)
					{
						if (match & REPLY_SERVICE_TYPE_PTR)
						{
							...
							{
								LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: service type PTR"LWIP_NEW_LINE));
						...
						if (match & REPLY_SERVICE_NAME_PTR)
						{
							...
							if (res == ERR_OK && mdns_domain_eq(&known_ans, &my_ans))
							{
								LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: service name PTR"LWIP_NEW_LINE));
				...
				else if (match & REPLY_SERVICE_SRV)
				{
					...
					do
					{
						...
						LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: SRV"LWIP_NEW_LINE));
				...
				else if (match & REPLY_SERVICE_TXT)
				{
					...
					{
						LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Skipping known answer: TXT"LWIP_NEW_LINE));
 ...
 */
static void mdns_handle_response(struct mdns_packet *pkt, mdns_client_t *mdnsClient )
{
//	int i = 0;
	...
	while (pkt->questions_left)
	{
		...
		if (res != ERR_OK)
		{
			LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Failed to parse question, skipping response packet"LWIP_NEW_LINE));
			return;
		}
	}
	...
	while (pkt->answers_left)
	{
		...
		i++;
		LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: read #%d Answer"LWIP_NEW_LINE, i));
		res = mdns_read_answer(pkt, &ans);
		if (res != ERR_OK)
		{
			LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Failed to parse answer, skipping response packet"LWIP_NEW_LINE));
			return;
		}

		LWIP_DEBUGF(MDNS_DEBUG, ("MDNS: Answer #%d for domain(%d) '", i, ans.info.domain.length));
#if MDNS_DEBUG
		mdns_domain_debug_print(&ans.info.domain);
#endif
		LWIP_DEBUGF(MDNS_DEBUG, ("' type %d class %d, rdLength:%d, rdOffset:%d"LWIP_NEW_LINE, ans.info.type, ans.info.klass, ans.rd_length, ans.rd_offset));
		if(ans.rd_length > 0)
		{
			CONSOLE_DEBUG_MEM(pkt->pbuf->payload + ans.rd_offset , (uint32_t)ans.rd_length, (uint32_t) ans.rd_offset, "MDNS Reponse Packet");
			printf("STR:'%s'"LWIP_NEW_LINE, (char *)(pkt->pbuf->payload + ans.rd_offset) );
		}
	}

	CONSOLE_DEBUG_MEM(pkt->pbuf->payload, (uint32_t)pkt->pbuf->len, 0, "MDNS Response Packet2");

#else

	mdnsClientParseAnswer(mdnsClient, pkt);
#endif
...
//	CONSOLE_DEBUG_MEM(p->payload, (uint32_t)p->tot_len, 0, "RECV MDNS Data");
#if 0
	CONSOLE_DEBUG_MEM(p->payload, (uint32_t)p->len, 0, "RECV MDNS Data");
#endif
	...
	{
		mdns_handle_response(&packet, client);
	}
	else
	{
 ...
 */
void mdns_resp_init(void *_client)
{
	err_t res;
...
err_t mdns_resp_add_netif(struct netif *netif, const char *hostname, u32_t dns_ttl)
{
	...
	/* send out announcement, only with hostname and IP */

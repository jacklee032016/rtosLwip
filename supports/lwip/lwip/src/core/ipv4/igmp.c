...
void igmp_init(void)
{
	LWIP_DEBUGF(IGMP_DEBUG, ("igmp_init: initializing"LWIP_NEW_LINE));
	LWIP_DEBUGF(IGMP_DEBUG, ("IGMP initializing"LWIP_NEW_LINE));

	IP4_ADDR(&allsystems, 224, 0, 0, 1);/* local all-hosts group */
	IP4_ADDR(&allrouters, 224, 0, 0, 2);	/* local all-routers group */

	EXT_INFOF(("IGMP start..."));
}
...
err_t igmp_start(struct netif *netif)
{
	...
	LWIP_DEBUGF(IGMP_DEBUG, ("igmp_start: starting IGMP processing on if %p"LWIP_NEW_LINE, (void*)netif));
	...
	if (group != NULL)
	{
		...
		if (netif->igmp_mac_filter != NULL)
		{
			...
			LWIP_DEBUGF(IGMP_DEBUG, (") on if %p"LWIP_NEW_LINE, (void*)netif));
...
void igmp_report_groups(struct netif *netif)
{
	...
	LWIP_DEBUGF(IGMP_DEBUG, ("igmp_report_groups: sending IGMP reports on if %p"LWIP_NEW_LINE, (void*)netif));
...
static struct igmp_group *igmp_lookup_group(struct netif *ifp, const ip4_addr_t *addr)
{
	...
	LWIP_DEBUGF(IGMP_DEBUG, (" on if %p"LWIP_NEW_LINE, (void*)ifp));
...
void igmp_input(struct pbuf *p, struct netif *inp, const ip4_addr_t *dest)
{
	...
	if (p->len < IGMP_MINLEN)
	{
		...
		LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: length error"LWIP_NEW_LINE));
		return;
	}
	...
	LWIP_DEBUGF(IGMP_DEBUG, (" on if %p"LWIP_NEW_LINE, (void*)inp));
	...
	if (inet_chksum(igmp, p->len))
	{
		...
		LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: checksum error"LWIP_NEW_LINE));
		return;
	}
	...
	if (!group)
	{
		...
		LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: IGMP frame not for us"LWIP_NEW_LINE));
		return;
	}
	...
	switch (igmp->igmp_msgtype)
	{
		case IGMP_MEMB_QUERY:
			...
			{/* THIS IS THE GENERAL QUERY */
				LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: General IGMP_MEMB_QUERY on \"ALL SYSTEMS\" address (224.0.0.1) [igmp_maxresp=%i]"LWIP_NEW_LINE, (int)(igmp->igmp_maxresp)));

				if (igmp->igmp_maxresp == 0)
				{
					IGMP_STATS_INC(igmp.rx_v1);
					LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: got an all hosts query with time== 0 - this is V1 and not implemented - treat as v2"LWIP_NEW_LINE));
			...
			{/* IGMP_MEMB_QUERY to a specific group ? */
				if (!ip4_addr_isany(&igmp->igmp_group_address))
				{
					...
					if (ip4_addr_cmp(dest, &allsystems))
					{
						...
						LWIP_DEBUGF(IGMP_DEBUG, (" using \"ALL SYSTEMS\" address (224.0.0.1) [igmp_maxresp=%i]"LWIP_NEW_LINE, (int)(igmp->igmp_maxresp)));
					...
					{
						LWIP_DEBUGF(IGMP_DEBUG, (" with the group address as destination [igmp_maxresp=%i]"LWIP_NEW_LINE, (int)(igmp->igmp_maxresp)));
					}
		...
		case IGMP_V2_MEMB_REPORT:
			LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: IGMP_V2_MEMB_REPORT"LWIP_NEW_LINE));
		...
		default:
			LWIP_DEBUGF(IGMP_DEBUG, ("igmp_input: unexpected msg %d in state %d on group %p on if %p"LWIP_NEW_LINE, igmp->igmp_msgtype, group->group_state, (void*)&group, (void*)inp));
			IGMP_STATS_INC(igmp.proterr);
			break;
	}
...
#if 0
	LWIP_ERROR(("igmp_joingroup: attempt to join non-multicast address:%s", inet_ntoa(*(ip_addr_t *)groupaddr) ), ip4_addr_ismulticast(groupaddr), return ERR_VAL;);
#else
...
err_t igmp_joingroup_netif(struct netif *netif, const ip4_addr_t *groupaddr)
{
	...
	if (group != NULL)
	{
		...
		{
			LWIP_DEBUGF(IGMP_DEBUG, ("igmp_joingroup_netif: join to group not in state IGMP_GROUP_NON_MEMBER"LWIP_NEW_LINE));
		}
		else
		{/* OK - it was new group */
	...
	{
		LWIP_DEBUGF(IGMP_DEBUG, ("igmp_joingroup_netif: Not enough memory to join to group"LWIP_NEW_LINE));
		return ERR_MEM;
	}
}
...
err_t igmp_leavegroup_netif(struct netif *netif, const ip4_addr_t *groupaddr)
{
	...
	if (group != NULL)
	{
		...
		if (group->use <= 1)
		{
			...
			{
				LWIP_DEBUGF(IGMP_DEBUG, ("igmp_leavegroup_netif: sending leaving group"LWIP_NEW_LINE));
	...
	{
		LWIP_DEBUGF(IGMP_DEBUG, ("igmp_leavegroup_netif: not member of group"LWIP_NEW_LINE));
		return ERR_VAL;
	}
}
...
static void igmp_send(struct netif *netif, struct igmp_group *group, u8_t type)
{
	...
	{
		LWIP_DEBUGF(IGMP_DEBUG, ("igmp_send: not enough memory for igmp_send"LWIP_NEW_LINE));
